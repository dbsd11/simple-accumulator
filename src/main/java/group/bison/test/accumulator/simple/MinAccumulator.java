 ...
 */
@Slf4j
public class MinAccumulator<T> extends AbstractAccumulator<T, Object> implements Accumulator<T, Object>, Comparator<Object> {
    ...
    public int offer(T data) {
        ...
        String minKey = accumulatorKeyList.stream().map(accumulatorKey -> String.valueOf(dataJSON.getOrDefault(accumulatorKey, accumulatorKey))).reduce(StringUtils::join).get();

        Object minVal = PersistTool.getObj(minKey, null);
        if (compare(minVal, parameter) > 0) {
            minVal = parameter;
        }

        PersistTool.setObj(minKey, minVal);
        return 1;
    }
